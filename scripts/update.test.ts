...
function updateFixture(): {
	...
	const marketplaceRoot = join(codeHome, ".tmp", "marketplaces", "agent-plugin-playground")
	...
	writeFileSync(
		...
		`[marketplaces.agent-plugin-playground]\nlast_revision = "${priorCommit}"\nsource_type = "git"\nsource = ${JSON.stringify(repositoryRoot)}\nref = "v0.1.0"\n\n[plugins."agent-plugin-playground@agent-plugin-playground"]\nenabled = true\n`,
	)
	...
	writeFileSync(
		...
		JSON.stringify({
			...
			plugins: {
				installed: [
					{
						pluginId: "agent-plugin-playground@agent-plugin-playground",
						name: "agent-plugin-playground",
						marketplaceName: "agent-plugin-playground",
						version: "0.1.0",
						installed: true,
						enabled: true,
...
function nativeUpdateFixture(): {
	...
	nativeCodexJson(
		["plugin", "add", "agent-plugin-playground@agent-plugin-playground", "--json"],
		project,
		environment,
	)
