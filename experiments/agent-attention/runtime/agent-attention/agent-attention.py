...
def validate_event_binding(
	value: Any, identifier: str, *, document_name: str
) -> dict[str, Any]:
	"""Reprove one claim or receipt against its exact event key."""
	required = ("approval_meaning", "event_id", "reminder_id", "thread_id")
	if not isinstance(value, dict) or any(
		not isinstance(value.get(field), str) or not value[field] for field in required
	):
		raise ContractError(f"{document_name} lacks the exact event binding")
...
def watch(args: argparse.Namespace) -> dict[str, Any]:
	...
	while True:
		...
		args.command_timeout_seconds = remaining
		result = poll(args)
		polls += 1
...
def record_delivery(args: argparse.Namespace) -> dict[str, Any]:
	...
	state_dir: Path = args.state_dir
	identifier = validate_event_id(args.event_id)
	claim_path = state_dir / "claims" / f"{identifier}.json"
	receipt_path = state_dir / "receipts" / f"{identifier}.json"
	if receipt_path.exists():
	...
	return base_result("recorded", changed=True, event_id=identifier)
...
def doctor(args: argparse.Namespace) -> dict[str, Any]:
	...
	return base_result(
		...
		next_safe_action=(
			"submit or poll an approval gate"
			if ready
